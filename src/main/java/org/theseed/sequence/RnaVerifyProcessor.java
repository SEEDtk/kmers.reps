 ...
 *
 * The genome list will be taken from the standard input, which must be a tab-delimited file. The genomes will be
 * processed in batches for performance.
 *
 * The output will only list the genomes that are considered bad. A genome is bad if it has less than 2 eligible
 * SSU rRNAs or if the maximum distance is 0.5 or more.
 *
...
public class RnaVerifyProcessor extends BaseReportProcessor {
    ...
    // FIELDS
    /** logging facility */
    private static final Logger log = LoggerFactory.getLogger(RnaVerifyProcessor.class);
    /** input file reader */
